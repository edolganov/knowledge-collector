...
public class TreeUtil {
	...
	public static DefaultMutableTreeNode[] getCurrentNodes(JTree tree) {
		TreePath[] paths = tree.getSelectionPaths();
		if(paths == null) return null;
		
		DefaultMutableTreeNode[] out = new DefaultMutableTreeNode[paths.length];
		for (int i = 0; i < paths.length; i++) {
			out[i] = (DefaultMutableTreeNode)paths[i].getLastPathComponent();
		}
		return out;
	}
	...
	public static DefaultMutableTreeNode addChild(JTree tree, DefaultMutableTreeNode parent, Object userObject, Class<?> downRank) {
		DefaultMutableTreeNode child =createNode(userObject);
		addChild(tree, parent, child, downRank);
		return child;
	}
	
	public static DefaultMutableTreeNode createNode(Object userObject){
